...
void Scan(){
  if ((status = rfid.findTag(&card_type)) == STATUS_OK &&
    ...
    if ((status = rfid.readTagSN(sn, &snBytes)) == STATUS_OK) {
      ...
      x=msg.buffer[4];
      y=msg.buffer[5];
      if(x==1 && y==4) treasure[0]=true;
      if(x==2 && y==0) treasure[1]=true;
      if(x==1 && y==3) treasure[2]=true;
      Serial.println("Recv:");
      Serial.print(x);
      Serial.print(",");
      Serial.print(y);
...
void loop()
{
  ...
  if(mode==1){
    ...
    if(DisLeft()>25 || DisRight()>25){
      ...
      delay(210);
      Scan();
      if(x==0 && y==4 && !treasure[0]){
        TurnLeft();
        MoveFront(180);
        delay(100);
      }
      else if(x==2 && y==1 && !treasure[1]){
        TurnRight();
        MoveFront(180);
        delay(100);
      }
      else if(x==2 && y==3 && !treasure[2]){
        TurnLeft();
        MoveFront(180);
        delay(100);
      }
...
//        while(1);
    ...
    if(DisFront()>25 && DisLeft()>20){
        MoveFront(180);
        delay(500);
        mode=1;
    }
    else if(DisFront()<15 && DisLeft()>20){
      if(DisFront()<15 && DisLeft()>20)
        TurnLeft();
        mode=1;
    }
