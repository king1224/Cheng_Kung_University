...
void Scan(){
  if ((status = rfid.findTag(&card_type)) == STATUS_OK &&
    ...
    if ((status = rfid.readTagSN(sn, &snBytes)) == STATUS_OK) {
      ...
      x=msg.buffer[4];
      y=msg.buffer[5];
      if(x==5 && y==2) treasure[0]=true;
      if(x==4 && y==6) treasure[1]=true;
      if(x==5 && y==3) treasure[2]=true;
      Serial.println("Recv:");
      Serial.print(x);
      Serial.print(",");
      Serial.print(y);
...
void loop()
{
  ...
  if(mode==1){
    ...
    if(DisLeft()>25 || DisRight()>25){
      MoveFront(180);
      delay(230);
      Scan();
      if(x==6 && y==2 && !treasure[0]){
        TurnLeft();
      }
      else if(x==4 && y==5 && !treasure[1]){
        TurnRight();
        MoveFront(180);
        delay(100);
      }
      else if(x==4 && y==3 && !treasure[2]){
        TurnLeft();
        MoveFront(180);
        delay(100);
      }
      
      if(DisFront()>20 && DisLeft()>25){
        TurnLeft();
        MoveFront(180);
        delay(100);
      }
        ...
        float L = DisLeft();
        float R = DisRight();
        if(F<20 && R>20 && R<50 && R<L){
          TurnRight();
        }
        else if(F<20 && L>20 && L<50){
          TurnLeft();
        }
      }
...
//        while(1);
    ...
    else if(DisRight()<6.5 || (DisLeft()>11 && DisLeft()<20)){
      MoveLeft_1M(180);
      delay(50);
      MoveFront(180);
      delay(100);
    ...
    if(DisFront()>25 && DisLeft()>25){
        MoveFront(180);
        delay(500);
        mode=1;
    }
    else if(DisFront()<15 && DisLeft()>25){
      if(DisFront()<15 && DisLeft()>25)
        TurnLeft();
        mode=1;
    }
    else if(DisFront()<15 && DisRight()>25 && DisLeft()<20){
      if(DisFront()<15 && DisRight()>25 && DisLeft()<20)
        TurnRight();
        mode=1;
    }
    else if(DisFront()<12 && DisRight()<15 && DisLeft()<15){
